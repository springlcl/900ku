  ...
  $.widget('mf.manifest', {
    ...
    add: function (data, $mpItem, clearInputValue, initial) {
      ...
      for (var i = 0, length = values.length; i < length; i++) {
        ...
        /***************************二次开发开始******************************/
          var display = options.formatDisplay.call($input, value, $item, $mpItem),
              remove = options.formatRemove.call($input, $remove, $item),
              value = options.formatValue.call($input, value, $value, $item, $mpItem);
          $item.html(display);
    ...
    _resizeInput: function () {
      ...
      textWidth = self._measureText('使用英文半角,及;分隔属性---');
