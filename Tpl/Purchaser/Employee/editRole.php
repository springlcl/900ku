    ...
    <div class="right_content fl">
        <div class="right_con_down bg-fff invoice_0505">
            <div class="cont_lump cont_lump_Customer staffSetup">
                ...
                <form action="{:U('editRole')}" method="post" >
                    <ul class="lump1">
                        <li class="">
                            <label class="text" ><i>*</i>角色名称:</label>
                            <input name="name" type="text " class="input" value="{$role.role_name}">
                            <input name="rid" type="hidden" value="{$Think.get.rid}"
                        </li>
                        <li>
                            <label class="text" ><i>*</i>角色描述:</label>
                            <input name="desc" type="text" value="{$role.description}">
                        </li>
                            ...
                            <li>
                                ...
                                <dl class="checks_box" parent="{$per.per_id}">
                                    <volist name="per['son']" id="p">
                                        <dd><input name="per[]" type="checkbox" value="{$p.per_id}" id="per{$p.per_id}" <if condition="in_array($p['per_id'],$pers)">checked="checked"</if>/><label <eq name="p['per_show']" value="1">class="col-lan"</eq> for="per{$p.per_id}">{$p.per_name}</label></dd>
                                    </volist>
                                </dl>
