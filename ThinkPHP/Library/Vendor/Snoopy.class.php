...
class Snoopy
{
    ...
    /*======================================================================*\
        Function:   fetchthelinks 自定义：获取文章列表页指定区域的文章链接
        Purpose:    fetch the links from a web page
        Input:      $URI    where you are fetching from
        Output:     $this->results  an array of the URLs
    \*======================================================================*/

    function fetchthelinks($URI,$statr,$end,$urlRule='/.+\/\d+.html$/',$code='utf8')
    {
        if ($this->fetch($URI) !== false)
        {
            // 过滤html标签
            ...
            */
            ...
            if($code == 'gbk')
            {
                $str = iconv('gbk', 'utf-8', $str); //如果是 utf-8 的就不需要了
            }
            $partern = '/('.$statr.')(.*)('.$end.').*?/';
            $match = array();
            ...
            if(!empty($match[2][0]))
            {
                ...
                {
                    if (preg_match_all($urlRule,$tmp,$match))
                    {
                        $urls[]=$match[0][0];
                    }
                }
                $this->results = array_unique($urls);
                return $this;
            }else{
                $this->results = array();
                return $this;
    ...
    /*======================================================================*\
        Function:   fetchthelinks 自定义：获取文章内容、标题及描述
        Purpose:    fetch the links from a web page
        Input:      $URI    where you are fetching from
        Output:     $this->results  an array of the URLs
    \*======================================================================*/
...
function getContent($url,$statr,$end,$code='utf8')
{
    if($this->fetch($url))
    {
        $str=preg_replace("/[\t\n\r]+/","",$this->results);
        ...
        //获取标题
        $inarr = array();
        preg_match("/<title>(.*)<\/title>/isU",$str,$inarr);
       if(isset($inarr[1]))
        {
             $data['title'] = $inarr[1];
        }
        //获取摘要
        $inarr = array();
        preg_match("/<meta[\s]+name=['\"]description['\"] content=['\"](.*)['\"]/isU",$str,$inarr);
        if(isset($inarr[1]))
        {
            $data['description'] = $inarr[1];
        }

        $partern= '/('.$statr.')(.*)('.$end.').*?/';
        preg_match_all($partern,$str,$match);

        $data['content'] = $match[2][0];
