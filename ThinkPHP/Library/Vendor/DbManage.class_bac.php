...
@ob_end_flush();
    ...
    function __construct() {
       ...
       // 数据库编码方式
        $db->query ( 'SET NAMES ' . $this->charset ,$db);
    ...
    function backup($tablename = '', $dir, $size) {
        ...
        // 只备份某个表
        if (! empty ( $tablename )) {
            if(@mysql_num_rows($db->query("SHOW TABLES LIKE '".$tablename."'")) == 1) {
             } else {
            ...
            // 字段数量
            $fields = $db->getFields($tablename);//来获取该表所有字段信息
            $num_fields = count($fields);//统计字段数
            ...
            $this->_showMsg('正在备份');
            // 备份全部表
            if ($tables = $db->query ( "show table status from " . $this->database )) {
            ...
            while ( $table ) {
                // 获取表名
                $tablename = $table [0]['tables_in_900ku'];
                // 获取表结构
                $sql .= $this->_insert_table_structure ( $tablename );
                $data = $db->query ( "select * from " . $tablename );
    ...
    function __destruct() {
        ...
        if($this->db){
            $db->query ( "unlock tables", $this->db );
            mysql_close ( $this->db );
        }
    }
